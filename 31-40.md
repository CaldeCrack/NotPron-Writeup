...
I first wanted to decipher the image itself, I thought that it was somewhat similar to the weekdays abbreviations but not quite, I googled Di Mi Do Fr and got that I was right but in german, which are Mo, Di, Mi, Do, Fr, Sa, So (starting on Monday).

Considering the image has something to do with dates I thought that the title seemed like one, January 8th, 1935, which using Google revealed that its the birthday of Elvis Presley, so with `elvis.htm` we get to the next page.

## [Interlude - Elvis](https://notpron.com/notpron/nomeaning/elvis.htm)

Before the next level we are interrupted by a page with some Elvis related content, just press the 'next' button to continue.

## [Level 38 - 4th?](https://notpron.com/notpron/nomeaning/elvis2.htm)

From the title and the photo I guess we need to follow the pattern, in the image there are three objects (I guess we just skip the flashlight it doesn't really seem part of the puzzle), coffee, a martini glass and an email.

In the source there is also the comment `<!--MSN-->` and the name of the image is `38messenger.jpg` which I guess it somehow relates to some messaging app but right now I don't see the connection and finally the hint to use Google.

At first I just thought of searching 'coffee martini email' and variations of it but it didn't work. I searched around to see what MSN meant and it was Microsoft Network but this didn't help that much. Then I just added MSN to the previous prompt and it throwed a page with a cheat sheet for emojis used in Gmail, Yahoo! Messenger and most importantly Windows Live Messenger (considering the image name and MSN comment).

I wasn't even born when these came out so it didn't cross my mind but with I noticed the aliases to emojis and there were exactly for the three in the image, so getting the fourth one was easy:
...
Actually using `beer.htm` says that we should try with the F instead, so with `flower.htm` we get it (I tried with red and rose but neither worked so I guessed flower).

## [Level 39 - Negenendertig](https://notpron.com/notpron/nomeaning/flower.htm)

Well now another 'pron' image, from the page my first thought was to search what the title meant, I thought it was maybe in some other language and effectively it was 39 in Dutch but right now this doesn't say anything to me.

From the image itself I thought of the typical joke of writing 'boobs' in the calculator upside down considering the content but no matter what I put in the url it didn't work.

Then in the source I saw the comment `<!--53;51;49;56;48;48;56; got an alternative again?-->` and my immediate thought was that these numbers were in ascii, translating them resulted in '5318008', which I recognized would be 'boobies' in a calculator upside down which is the answer, so I guess I just slightly failed before (in my defense I'm not an english native).

Also there is new background audio but I guess it didn't matter this time.

## [404 - 404: Page not found](https://notpron.com/notpron/nomeaning/boobies.htm)

Another interlude where you just have to click the boob to pass onto the next level.

## [Level 40 - Cuarenta](https://notpron.com/notpron/nomeaning/stfuandgetmetothefuckingscreen40.htm)

Finally something in my native language (cuarenta is forty in spanish), I feel like this is sort of building up to a puzzle in the future regarding this numbers in different languages.

This level was pretty easy, in the image just barely visible inside the 4 in 40 is something, zooming into it says 'next', so just trying with `next.htm` worked.
