...
After this level I think if it seems even remotely possible I'll try to use pron in everything.
...
There is absolutely nothing else in the page aside from the image, the image itself its someone and in its eye I think there is some kind of lamb skull, I guess this level is just about finding the url.

After a long while I used `david.htm`, since he is the Notpron creator and the title was pointing towards some creator, but its not the solution although at least I got some kind of hint I guess, the following quote 'yeah thats me :D now try to look in my face, where could it be?'.

Actually I got an extra hint using `davidm.htm`, since that seems to be his online username, and got the exact same message as before but with an extra line 'dont try to photoshop that picture, useless.....', I guess we can discard that now.

Now I got another page that isn't the solution and it isn't even a hint for this level, but I'll still note it down to keep it in mind for the future, using `germany.htm` shows the comment 'If you come from Level 30, ignore this. It's got nothing to do with it. For others: and where in germany'.

I gotta admit for this level I used the notpron forums for some hints, the main one was leave the level but not notpron, and also no backtracking other levels, so I though that maybe it would be in the home page.

In it I pressed tab to see all clickables and got some strange shapes and whatnot but nothing useful, so I went back to the forums and noticed that some people were talking about credits and finding where could the creator be, so I thought that maybe there was a credits page, and sure enough the url `https://notpron.com/notpron/credits.htm` existed.

But this didn't solve it still, seeing all the source code didn't give me a direct answer, I tried putting some of the main names from the 'Impressum' but nothing.

Finally, after a while I realized that the creator's image file name and url weren't matching, actually its one of the first things I noticed but didn't give it that much importance, but when I returned to it I thought of, like in level 18, changing the odd 'b' `http://deathball.net/pics/team_davidb.jpg` in the file to other letters, actually 'c' existed but it was just a smaller version of the `team_david.jpg` photo, but then on 'm' I got jumpscared by the image and got the solution (just in case no other letters had an image).

## [Level Ad - Shameless commercial break which really annoys you!](https://notpron.com/notpron/screen30/davidandhism.htm)

Its just like an ad for a Notpron book, at the end of the page is a button to the actual next level, it could maybe still have some relevant info for latter levels so its worth saving.
